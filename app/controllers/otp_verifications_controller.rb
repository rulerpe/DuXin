class OtpVerificationsController < ApplicationController
  ...
  def verify_user_otp
    ...
    if is_verified
      ...
      cookies.signed[:auth_token] = {value: token, httponly: true, same_site: :none, secure: Rails.env.production?, expires: 1.week.from_now}
      cookies.signed[:test] = {value: "test"}
      render json: { message: "Phone number verified successfully.", token: token, user: user }, status: :ok
    else
